...
export module sigslot;

namespace sigslot {

template <typename, typename...> class signal_base;
...
template <typename T> std::weak_ptr<T> to_weak(std::weak_ptr<T> w) { return w; }

template <typename T> std::weak_ptr<T> to_weak(std::shared_ptr<T> s) {
  return s;
}
...
// void_t from c++17
template <typename... T> using void_t = typename detail::voider<T...>::type;
...
template <typename T> constexpr bool is_func_v = std::is_function<T>::value;
...
template <typename S> constexpr bool is_signal_v = detail::is_signal<S>::value;
...
struct object_pointer<
    T, std::enable_if_t<!trait::is_pointer_v<T> && !trait::is_weak_ptr_v<T> &&
...
/**
 * A spin mutex that yields, mostly for use in benchmarks and scenarii that
 * invoke slots at a very high pace. One should almost always prefer a standard
 * mutex over this.
 */
struct spin_mutex {
...
public:
  ...
  copy_on_write() : m_data(new payload) {}

  template <typename U>
  explicit copy_on_write(
      U &&x,
      std::enable_if_t<!std::is_same<std::decay_t<U>, copy_on_write>::value> * =
          nullptr)
  ...
  copy_on_write(copy_on_write &&x) noexcept : m_data(x.m_data) {
    x.m_data = nullptr;
  }
...
private:
  bool unique() const noexcept { return m_data->count == 1; }
...
template <typename T> const T &cow_read(copy_on_write<T> &v) {
  return v.read();
}
...
/**
 * std::make_shared instantiates a lot a templates, and makes both compilation
 * time and executable size far bigger than they need to be. We offer a
 * make_shared equivalent that will avoid most instantiations with the following
 * tradeoffs:
 * - Not exception safe,
...
inline std::shared_ptr<B> make_shared(Arg &&...arg) {
  return std::static_pointer_cast<B>(
      std::make_shared<D>(std::forward<Arg>(arg)...));
}
#endif
...
protected:
  ...
  group_id group() const { return m_group; }

private:
  template <typename, typename...> friend class ::sigslot::signal_base;
...
/**
 * connection_blocker is a RAII object that blocks a connection until
 * destruction
 */
...
public:
  ...
  bool valid() const noexcept { return !m_state.expired(); }
...
protected:
   ...
   *
   * To avoid invocation of slots on a semi-destructed instance, which may
   * happen in multi-threaded contexts, derived classes should call this method
   * in their destructor. This will ensure proper disconnection prior to the
   * destruction.
   */
  void disconnect_all() {
...
template <typename...> class slot_base;

template <typename... T> using slot_ptr = std::shared_ptr<slot_base<T...>>;
 ...
 */
template <typename... Args> class slot_base : public slot_state {
public:
  using base_types = trait::typelist<Args...>;

  explicit slot_base(cleanable &c, group_id gid)
      : slot_state(gid), cleaner(c) {}
  ...
  template <typename C> bool has_callable(const C &c) const {
    auto p = get_callable();
  ...
  template <typename O> bool has_object(const O &o) const {
...
private:
  template <typename U> bool check_class_type() const {
...
public:
  ...
  constexpr slot(cleanable &c, F &&f, Gid gid)
      : slot_base<Args...>(c, gid), func{std::forward<F>(f)} {}
...
public:
  ...
  constexpr slot_pmf(cleanable &c, F &&f, P &&p, group_id gid)
      : slot_base<Args...>(c, gid), pmf{std::forward<F>(f)},
        ptr{std::forward<P>(p)} {}
 ...
 * the life of a supplied object through a weak pointer in order to
 * automatically disconnect the slot on said object destruction.
 */
...
class signal_base final : public detail::cleanable {
  ...
  using cow_type =
      std::conditional_t<is_thread_safe<L>::value, detail::copy_on_write<U>, U>;
...
public:
  ...
  connect(Pmf &&pmf, Ptr &&ptr, group_id gid = 0) {
    ...
    auto s =
        make_slot<slot_t>(std::forward<Pmf>(pmf), std::forward<Ptr>(ptr), gid);
    connection conn(s);
    add_slot(std::move(s));
  ...
  /**
   * Overload of connect for lifetime object tracking and automatic
   * disconnection.
   *
  ...
  /**
   * Overload of connect for lifetime object tracking and automatic
   * disconnection with additional connection management.
   *
   * The callable's first argument must be of type connection. The callable
   * can manage its own connection through this argument.
   *
   ...
   * pointer concept, by implementing the ADL-detected conversion function
   * to_weak().
   *
   * This overload covers the case of a standalone callable and unrelated
   * trackable object.
   *
   * Note: only weak references are stored, a slot does not extend the lifetime
   * of a supplied object.
  ...
  std::enable_if_t<trait::is_callable_v<arg_list, Callable> &&
                       trait::is_weak_ptr_compatible_v<Trackable>,
                   connection>
   ...
   * Safety: Thread-safety depends on locking policy.
   *
   * If the callable is a free or static member function, this overload is
   * always available. However, RTTI is needed for it to work for pointer to
   * member functions, function objects or and (references to) lambdas, because
   * the C++ spec does not mandate the pointers to member functions to be
   * unique.
   *
   * @param c a callable
   ...
   *
   * Effect: Disconnects all the slots bound to the callable and object in
   * argument. Safety: Thread-safety depends on locking policy.
   *
...
/**
 * Freestanding connect function that defers to the `signal_base::connect`
 * member.
 */
export template <typename Lockable, typename Arg, typename... T, typename... Args>
 ...
 */
export template <typename Lockable1, typename Lockable2, typename... T1,
          typename... T2, typename... Args>
connection connect(signal_base<Lockable1, T1...> &sig1,
                   signal_base<Lockable2, T2...> &sig2, Args &&...args) {
  return sig1.connect(
      detail::signal_wrapper<signal_base<Lockable2, T2...>>{
          std::addressof(sig2)},
 ...
 */
export template <typename... T> using signal = signal_base<std::mutex, T...>;
